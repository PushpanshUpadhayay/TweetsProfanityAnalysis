...
def get_input_file(header, file_path, file_ext):
    ...
    if file_ext in ["txt", "text", "csv"]:
        ...
        input_file = pd.read_excel(file_path, header=header_option, engine="openpyxl")
    return input_file
...
if not os.path.isfile(tweet_file_path):
    ...
    if os.stat(tweet_file_path).st_size > 0:
        ...
        input_file["Num. Foul Words"] = input_file["Foul Words"].apply( lambda x: len(re.findall(r'\w+', x)))

        input_file['Total Word Count'] = input_file['Tweets'].apply(lambda x: len(re.findall(r'\w+', x)))
        ...
        input_file.to_csv("profanity_analysis.csv", index=False)